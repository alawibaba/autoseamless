...
class SeamlessBrowser:
  ...
  def login(self, loginCredentials):
    ...
    groupOrder = self.request('https://www.seamless.com/food-delivery/login.m',\
  ...
  def selectRestaurant(self, wk, restaurantSelector):
    ...
    if todaysTag is None:
      self.log("It looks like we either don't order today or it's too late to do so.\nSorry about that!")
      return False
    ...
    if len(desiredRestaurant) > 1:
      self.log("Warning: multiple restaurants matched -- taking the first one!")
  ...
  def addItemToOrder(self, desiredItem):
    ...
    pdata = ("ajaxCommand=29~0&29~0action=Save&29~0orderId=%s&" % self.orderID) + "&".join([x for x in map(lambda x: x.has_key('name') and x.has_key('value') and "29~0%s=%s" % (x['name'], x['value']), parsedItemPage.find('form')('input')) if x])
    
    addItemResponse = self.request("https://www.seamless.com/Ajax.m", postdata=pdata, updateURL=False)
  ...
  def checkout(self, phoneNumber="(617)555-3000"):
    ...
    pdata="goToCheckout=NO&TotalAlloc=%s00&LineId=&saveFavoriteCommand=Checkout.m&WhichPage=Meals&favoriteNameOriginal=&firstCheckOut=Y&acceptedBudgetWarning=N&AcceptedWarnings=N&acceptedFavoriteWarning=N&FavoriteSaved=N&UserSearchType=&ShowAddUser=N&deliveryType=Delivery&EcoToGoOrderId=%s&EcoToGoUserId=%s&OverageAllocationAmt=0&InfoPopupfavorite_name=&InfoPopupfavorite_saveType=&InfoPopupfavorite_orderId=%s&AllocationAmt1=%s&FirstName=&LastName=&NewAllocationAmt=&totalAllocated=$%s&AllocationComment=&typeOfCreditCard=&creditCardNumber=&CCExpireMonth=1&CCExpireYear=%d&creditCardZipCode=&CreditCardCVV=&OrderIdClicked=%s&FloorRoom=9&phoneNumber=%s&DeliveryComment=&EcoToGoOrder=Y&InfoPopup_name=Namethisfavorite&favoriteSaveMode=successWithOrderingMeals" % (alloc, self.orderID, self.userID, self.orderID, alloc, alloc, year, self.orderID, phoneNumber)
    
    checkoutResponse = self.request("https://www.seamless.com/Checkout.m", postdata=pdata, updateURL=False)
    ...
    if len(thanksMessage) < 1:
      self.log("Looks like the order failed for some reason -- probably exceeded the meal allowance.")
      return False
    ...
    return True
  def order(self, loginCredentials, phoneNumber, restaurantSelector, itemSelector, dryRun=False, wk=None):
...
if __name__ == "__main__":
  ...
  sys.exit(SeamlessBrowser(log).order(loginCredentials, "(617)555-3000", 
                          niSelect(re.compile("Tossed")),
                          niSelect(re.compile("Cayenne Shrimp Salad")), dryRun=True, wk="Thursday"))
